...
def test_clean_signal_shape():
    subject = '53'
    fmriprepped_data = FmriPreppedDataSet(example_data)
    clean_ts_= np.asarray(fmriprepped_data.clean_signal(subject, task = "rest", parcellation="schaefer", n_parcels=1000))

    assert clean_ts_.shape[0] == 1, "First dimension should be 1 (n_sessions)"
    assert clean_ts_.shape[2] == 1000, "Third dimension should be 1000 (n_parcels)"
...
def test_get_gradients():
    gradients, lambdas = get_gradients(example_data, subject = '53', n_components = 10, parcellation='schaefer', n_parcels = 1000, task = "rest", aligned = False)
...
def test_get_gradients_aligned():
    gradients, lambdas = get_gradients(example_data, '53', n_components = 10, parcellation='schaefer', n_parcels = 1000, task = "rest", aligned = True)
