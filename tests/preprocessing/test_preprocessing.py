...
def test_subject():
    raw_data = RawDataset(example_data)
    subjects = raw_data.subjects
    assert "17017" in subjects, "The subject is not on the list"
...
def test_clean_signal_shape():
    n_parcels = 1000
    fmriprepped_data = FmriPreppedDataSet(example_data)
    clean_ts_= np.asarray(fmriprepped_data.clean_signal(subject = fmriprepped_data.subjects[0], task = "rest"))
...
def test_conn_matrix():
    fmriprepped_data = FmriPreppedDataSet(example_data)
    conn_matrix = fmriprepped_data.get_conn_matrix(subject = fmriprepped_data.subjects[0], task = "rest", save = True)
    path_conn_matrix = os.path.join(f'{fmriprepped_data.data_path}', 'clean_data', f'sub-{fmriprepped_data.subjects[0]}', 'func', f'conn-matrix-sub-{fmriprepped_data.subjects[0]}-rest-schaefer1000.npy')
